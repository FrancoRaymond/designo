    ...
    <div className='bg-white w-full z-50 flex justify-between items-center py-4 px-2 sm:px-5 md:px-10 lg:px-24 fixed top-0'>
      ...
      >
        <ul className={`flex gap-8 ${size < 640 ? 'flex-col items-start px-2 h-fit w-full text-white' : 'flex-row text-black'}`}>
          <li className='hover:text-[var(--accent-color)] transition duration-300'><Link onClick={closeMenu} to="/about">OUR COMPANY</Link></li> 
          <li className='hover:text-[var(--accent-color)] transition duration-300'><Link onClick={closeMenu} to="/locations">LOCATIONS</Link></li>
          <li className='hover:text-[var(--accent-color)] transition duration-300'><Link onClick={closeMenu} to="/contacts">CONTACTS</Link></li>
        </ul>
      </motion.nav>
      ) : (
